...
pub fn get_diff_files(prev_commit: &str, next_commit: &str, git_dir: &str) -> Option<Vec<StatItem>> {
	log::debug!("[get_diff_files] prev_commit = {}, next commit = {}, git_dir = {}",
	...
	if git_res.is_err() {
		...
		log::error!("[get_diff_files] git diff stat command failed to start : {:?}", commanderr);
		return None;
	}
	...
	if stat_res.is_err() {
		...
		log::error!("[get_diff_files] git diff stat command parse error : {:?}", staterr);
		return None;
	}
