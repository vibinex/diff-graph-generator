...
use crate::{graph::gitops::get_hunks_all_files, utils::gitops::git_checkout_commit};
...
pub async fn generate_diff_graph(prev_commit: &str, next_commit: &str, git_dir: &str) -> Option<DiffGraph> {
    if let Some(mut hunk_diff_map) = get_hunks_all_files(prev_commit, next_commit, git_dir) {
        // get func defs for base commit for files in diff
        log::debug!("[generate_diff_graph] hunk diff map =======~~~~~~~~ {:#?}", &hunk_diff_map);
        let diff_graph_opt = process_hunk_diff(&mut hunk_diff_map, prev_commit, next_commit, git_dir).await;
        return diff_graph_opt;
    }
    return None;
}

async fn process_hunk_diff(hunk_diff_map: &mut HunkDiffMap, prev_commit: &str, next_commit: &str, git_dir: &str) -> Option<DiffGraph> {
    ...
    let added_files_defs = set_func_def_whole_file(git_dir, hunk_diff_map.added_files(), &mut def_identifier).await;
    ...
    let deleted_files_defs = set_func_def_whole_file(git_dir, hunk_diff_map.deleted_files(), &mut def_identifier).await;
...
async fn set_func_def_whole_file(git_dir: &str, added_deleted_files: Vec<&String>, def_identifier: &mut DefinitionIdentifier) -> HashMap<String, Vec<FunctionDefinition>> {
    // send to prompt object and get vec of all defs, which should be stored in diff map? db?
    let mut file_def_map = HashMap::new();
    let base_path = Path::new(git_dir);
