...
use super::{
    file_imports::ImportDefOutput,
    function_name::FunctionDefinition,
    gitops::HunkDiffLines,
    utils::{call_llm_api, detect_language, numbered_content, read_file, strip_json_prefix},
};
...
pub async fn function_calls_in_chunk(chunk: &str, func_name: &str) -> Option<FunctionCallChunk> {
    ...
    if func_call_input_res.is_err() {
        ...
        log::error!(
            "[function_calls_in_chunk] Error serializing func call input: {:?}",
            e
        );
        return None;
    }
    ...
    if prompt_response_opt.is_none() {
        log::error!(
            "[function_calls_in_chunk] Unable to call llm for chunk: {:?}",
            chunk
        );
        return None;
    }
    ...
    if deserialized_response.is_err() {
        ...
        log::error!(
            "[function_calls_in_chunk] Error in deserializing response: {:?}",
            e
        );
        return None;
    }
    let func_call_chunk: FunctionCallChunk =
        deserialized_response.expect("Uncuaght error in deserialized_response");
...
pub async fn function_calls_in_file(
    filepath: &PathBuf,
    func_name: &str,
) -> Option<Vec<FunctionCallChunk>> {
    ...
    for chunk in chunks {
        ...
        if func_call_chunk_opt.is_none() {
            log::debug!(
                "[function_calls_in_file] No function call in chunk for file: {:?}",
                filepath
            );
            continue;
        }
    ...
    if all_func_calls.is_empty() {
        log::debug!(
            "[function_calls_in_file] No function call in file: {:?}, {:?}",
            filepath,
            func_name
        );
        return None;
...
impl FunctionCallsOutput {
    ...
    pub fn trim_empty_function_calls(&mut self) {
        self.function_calls.retain(|func_call| {
            !func_call.function_name().is_empty() && func_call.function_name().len() > 3
        });
    }
}
        ...
        while let Some(current_def) = definition_iter.peek().cloned() {
            ...
            if call.line_number as usize >= current_def.line_number
                && (next_def_line_opt.is_none()
                    || (call.line_number as usize)
                        < next_def_line_opt.expect("Empty next_def_line_opt"))
            {
...
impl FunctionCallIdentifier {
    pub fn new() -> Option<Self> {
        ...
        if prompt_json_res.is_err() {
            log::error!(
                "[FunctionCallIdentifier/new] Unable to deserialize prompt_json: {:?}",
    ...
    pub async fn functions_in_file(
        ...
        filepath: &PathBuf,
        lang: &str,
    ) -> Option<FunctionCallsOutput> {
        // concatenate functioncallsoutput for all chunks
        let mut all_func_calls: FunctionCallsOutput = FunctionCallsOutput {
            function_calls: vec![],
            notes: None,
        };
        ...
        if file_contents_res.is_err() {
            log::error!(
                ...
                &filepath,
                file_contents_res.expect_err("Empty error in file_contents_res")
            );
            return None;
        }
        let file_contents = file_contents_res.expect("Uncaught error in file_contents_res");
        ...
        for chunk in chunks {
            let chunk_str = chunk.join("\n");
            log::debug!(
                "[FunctionCallIdentifier/functions_in_file] chunk = {}",
                &chunk_str
            );
            if let Some(mut func_calls) = self.functions_in_chunk(&chunk_str, filepath, lang).await
            {
                log::debug!(
                    "[FunctionCallIdentifier/functions_in_file] func_calls = {:?}",
                    &func_calls
                );
                all_func_calls
                    .function_calls
                    .append(&mut func_calls.function_calls);
            }
        }
    ...
    async fn functions_in_chunk(
        ...
        if prompt_response_opt.is_none() {
            log::error!(
                "[FunctionCallIdentifier/functions_in_chunk] Unable to call llm for chunk: {:?}",
                chunk
            );
            return None;
        }
        ...
        if deserialized_response.is_err() {
            let e = deserialized_response.expect_err("Empty error in deserialized_response");
            log::error!(
                "[FunctionCallIdentifier/functions_in_chunk] Error in deserializing response: {:?}",
                e
            );
            return None;
        }
        let mut func_calls: FunctionCallsOutput =
            deserialized_response.expect("Empty error in deserialized_response");
    ...
    pub async fn function_calls_in_hunks(
        &mut self,
        filepath: &PathBuf,
        lang: &str,
        diff_hunks: &Vec<HunkDiffLines>,
    ) -> Option<Vec<(HunkDiffLines, FunctionCallsOutput)>> {
        let file_contents_res = std::fs::read_to_string(filepath.clone());
        if file_contents_res.is_err() {
            log::error!(
        ...
        for diff_hunk in diff_hunks {
            ...
            let batch_size = 30;
            let mut hunk_calls: FunctionCallsOutput = FunctionCallsOutput {
                function_calls: vec![],
                notes: None,
            };
            for start in (diff_hunk.start_line().to_owned()..diff_hunk.end_line().to_owned())
                .step_by(batch_size)
            {
                let end = usize::min(start + batch_size, diff_hunk.end_line().to_owned() + 1);
                log::debug!(
                    "[function_calls_in_hunks] start = {:?}, end = {:?}",
                    &start,
                    &end
                );
                let safe_start = usize::min(start, numbered_content.len());
                let safe_end = usize::min(end, numbered_content.len());
                if safe_start >= safe_end {
                    log::warn!("[function_calls_in_hunks] Skipping invalid chunk range: start = {:?}, end = {:?}, content_len = {:?}", safe_start, safe_end, numbered_content.len());
                    continue;
                }
                let chunk_slice = &numbered_content[safe_start..safe_end];
                let chunk_str = chunk_slice.join("\n");
                if let Some(mut func_calls) =
                    self.functions_in_chunk(&chunk_str, filepath, lang).await
                {
                    log::debug!(
                        "[FunctionCallIdentifier/function_calls_in_hunks] chunk = {:?}",
                        &func_calls
                    );
                    hunk_calls
                        .function_calls
        ...
        log::debug!(
            "[FunctionCallIdentifier/function_calls_in_hunks] hunk_func_pairs = {:?}",
            &hunk_func_pairs
        );
...
pub fn function_calls_search(
    directory: &str,
    function_name: &str,
    lang: &str,
) -> Option<HashMap<String, Vec<(usize, String)>>> {
...
impl FunctionCallValidator {
    pub fn new() -> Option<Self> {
        let system_prompt_opt = read_file(&format!(
            "{}/prompt_function_call_validator",
            get_prompt_path()
        ));
        if system_prompt_opt.is_none() {
            log::debug!(
                "[FunctionCallValidator/new] Unable to read prompt_function_call_validator"
            );
            return None;
        }
        ...
        if sys_prompt_struct_res.is_err() {
            log::debug!(
                "[FunctionCallValidator/new] Unable to deserialize sys prompt: {:?}",
                sys_prompt_struct_res.expect_err("Empty error")
            );
            return None;
        }
        let sys_prompt_struct: FunctionCallValidatorPrompt =
            sys_prompt_struct_res.expect("Uncaught error in sys_prompt_struct_res");
        return Some(Self {
            prompt: sys_prompt_struct,
        });
    }

    pub async fn valid_func_calls_in_file(
        &mut self,
    ...
    async fn valid_func_calls(
        ...
        let func_call_validator_prompt_str =
            func_call_validator_prompt_res.expect("Uncaught error in import_def_prompt_str_res");
        let prompt_str = format!("{}\nOutput -", &func_call_validator_prompt_str);
        log::debug!(
            "[FunctionCallValidator/valid_func_calls] prompt_str: {}",
            &prompt_str
        );
        ...
        let call_validator_output: FunctionCallValidatorOutput =
            validator_res.expect("Uncaught error in import_def_res");
        log::debug!(
            "[FunctionCallValidator/valid_func_calls] import_def: {:?}",
            &call_validator_output
        );
