...
impl MermaidSubgraph {
    ...
    pub fn render_subgraph(&self, prev_commit: &str, next_commit: &str, git_url: &str, subgraph_map: &HashMap<String, MermaidSubgraph>) -> String {
        let mut all_nodes = Vec::new();
        for (_, node) in self.nodes() {
            all_nodes.push(node.render_node(prev_commit, next_commit, git_url, subgraph_map));
        }
...
impl MermaidNode {
    ...
    pub fn render_node(&self, prev_commit: &str, next_commit: &str, git_url: &str, subgraph_map: &HashMap<String, MermaidSubgraph>) -> String {
        let url_str = format!("\tclick {} href \"{}\" _blank",
            self.mermaid_id(), self.get_node_str(prev_commit, next_commit, git_url, subgraph_map));
    ...
    fn get_node_str(&self, prev_commit: &str, next_commit: &str, git_url: &str, subgraph_map: &HashMap<String, MermaidSubgraph>) -> String {
...
impl MermaidGraphElements {
    ...
    fn render_subgraphs(&self, prev_commit: &str, next_commit: &str, git_url: &str) -> String {
        format!("{}\n{}",
            self.subgraphs
                .values()
                .map(|subgraph| subgraph.render_subgraph(prev_commit, next_commit, git_url, &self.subgraphs))
                .collect::<Vec<String>>()
                .join("\n"),
    ...
    pub fn render_elements(&self, prev_commit: &str, next_commit: &str, git_url: &str) -> String {
        let all_elements_str = format!("{}\n{}", &self.render_subgraphs(prev_commit, next_commit, git_url), &self.render_edges(prev_commit, next_commit, git_url));
        all_elements_str
    }

    fn render_edges(&self, prev_commit: &str, next_commit: &str, git_url: &str) -> String {
